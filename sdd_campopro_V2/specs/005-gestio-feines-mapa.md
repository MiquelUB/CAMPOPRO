...
- **Almacenamiento Local Seguro de Multimedia en Disco Duro (Sin AWS S3):** Toda la información multimedia y documental reside directamente en los discos duros locales del servidor de la empresa bajo el árbol de rutas relativas:
  ...
  * Planos técnicos y vectores de red: `/docs/<empresa_id>/planols/vectorials/`  
  La base de datos almacena exclusivamente rutas relativas locales y metadatos técnicos, con aislamiento multi-inquilino mandatorio (RLS) y soberanía de datos RGPD.
...
- **RF-05:** EL SISTEMA exigirá la **Georreferenciación Obligatoria Previa de toda Orden de Trabajo**, quedando terminantemente prohibido agendar o planificar tareas sin coordenadas latitud/longitud contrastadas en base de datos:
  ...
  3. *Inaccesibilidad Física al Punto Cero (Cancela con Candado):* SI la cuadrilla llega a la cancela y encuentra el acceso físico bloqueado, el operario registrará la **Incidencia de Acceso Bloqueado** para activar el contacto urgente con el cliente, pero **la orden NO conmuta a Rojo en el mapa; únicamente el operario en la PWA puede estimar pulsar voluntariamente la pausa/parada del tiempo de faena**. Si existe un acceso alternativo, el operario continuará y el sistema actualizará las coordenadas sin penalización. Si el cliente acude con demora a abrir la cancela, **el tiempo de espera se computará y facturará directamente como hora ordinaria desde la hora de llegada original (fichaje en Punto Cero)**. Si el acceso resulta definitivamente inviable tras el tiempo de espera que determine humanamente el supervisor según la situación, el supervisor cancelará la orden (conmutando a Negro), reasignará la cuadrilla y **se liquidará al cliente el coste del desplazamiento**, tarificado en base a las tarifas horarias de mano de obra configuradas en Magatzem (Spec 004) como horas de cuadrilla (operario, oficial, ingeniero) invertidas en el trayecto.
...
- **RF-07:** EL SISTEMA representará los marcadores (*pins*) en el mapa mediante una simbología cromática diferenciada por entidad (Obras vs. Vehículos), estrictamente funcional e independiente del tema camaleónico del software:
  ...
  2. *Vehículos y Flota Móvil:*
     ...
     - *Rojo:* **Avería mecánica o inmovilizado** (vehículo averiado en carretera, pinchazo o parada de auxilio).
  ...
  4. *Cierre de Jornada con Incidencia Abierta Inconclusa:* Al finalizar la jornada laboral, la cuadrilla retorna a la base y el vehículo desactiva su telemetría GPS, pero **la Orden de Trabajo permanece en Rojo (Incidencia activa / Inconclusa)** en el mapa y en el sistema para su resolución y reprogramación prioritaria al día siguiente.
...
- **RF-10:** PRIORIDAD A LA VISIÓN LIMPIA DEL MAPA Y PANELES COLAPSABLES:
  ...
  2. **Prioridad y Coexistencia de Paneles:** El Drawer de incidencias se despliega automáticamente ante una nueva incidencia marcándola y enfocándola en el mapa, pero **permite coexistir con la consulta de cuadrillas y la secuencia de órdenes de la jornada sin bloqueo mutuo**: el supervisor puede alternar o mantener abiertos los paneles laterales según requiera para evaluar el impacto de la contingencia en las faenas restantes de la cuadrilla.
...
- **RF-32:** INCIDENCIAS DE HERRAMIENTAS, TRASPASOS CONDICIONALES, PROVEEDORES Y VEHÍCULO DE APOYO:
  ...
  5. *Responsabilidad e Imputación por Avería de Herramienta Prestada:*
     - Cuando una herramienta traspasada entre cuadrillas sufra rotura, deterioro o avería durante su utilización en la obra receptora, la responsabilidad material y el coste de reposición o reparación se imputará **estricta y objetivamente a la cuadrilla y operario que ostentaba la custodia física efectiva en el momento del siniestro**, según conste en el registro digital de traspaso formalizado en Magatzem (Spec 004). En ningún caso se trasladará la responsabilidad a la cuadrilla cedente original.
...
- **RF-34:** TRAZABILIDAD MULTINIVEL E IMPUTACIÓN DISCIPLINARIA OBJETIVA EN RRHH:
  ...
  3. *Imputación Disciplinaria Individualizada en RRHH (`/gestio/treballadors`) y Rectificación Administrativa:*
     - La responsabilidad se asignará de forma objetiva según la naturaleza de la negligencia:
       * *Siniestro o Choque Vial y Mecanismo de Rectificación de Conductor:* Se imputará inicialmente al **conductor registrado del vehículo** en la orden de marcha matinal. **Mecanismo de Rectificación Administrativa:** Si existiera discrepancia entre el conductor registrado en la app y quien conducía materialmente en el instante del siniestro, los roles `Secretaria / RRHH` y `Boss` dispondrán de un **formulario formal de edición y rectificación disciplinaria**, mediante el cual, previa comprobación del parte amistoso, atestado o declaraciones firmadas, se trasladará la imputación y responsabilidad de forma trazable al conductor real que pilotaba el vehículo.
...
- **Almacenamiento Local Seguro en Disco Duro del Servidor (Cero S3):** Todas las evidencias multimedia y documentales se alojan estrictamente en el disco duro local del servidor bajo el árbol `/data/<empresa_id>/...` y `/docs/<empresa_id>/...`, garantizando soberanía territorial estricta y cumplimiento del RGPD.
...
4. Georreferenciación obligatoria previa de todas las órdenes de trabajo con geovalla de 50 metros y protocolo de Punto Cero con cancela cerrada (sin paso automático a rojo; pausa voluntaria por operario; tiempo de espera determinado por supervisor facturado como hora ordinaria; cancelación liquida desplazamiento en base a tarifas horarias de mano de obra de Magatzem Spec 004).
5. Código cromático con distinción ontológica estricta: Órdenes (Naranja Pendiente, Azul En tránsito, Verde En faena, Rojo Incidencia activa en obra, Blanco Completada, Negro Cancelada) y Vehículos (Azul En tránsito con tarea, Lila Standby/tránsito sin tarea, Rojo Avería mecánica; Blanco y Negro no aplican a vehículos; fin de jornada con incidencia abierta mantiene la orden en Rojo y apaga el GPS del vehículo en base).
...
7. Coexistencia pacífica de paneles: el Drawer de incidencias y el panel de órdenes del día coexisten sin destrucción mutua de estado.
...
10. Gestión de imprevistos con el cliente: teléfono destacado para llamada urgente directa del supervisor si es crítico, y mandato constitucional Human-in-the-Loop donde el informe y presupuesto de Copilot requiere aprobación humana obligatoria antes de ser enviado a Telegram.
11. Inmutabilidad de partidas aprobadas: las partidas aceptadas por el cliente en Telegram quedan bloqueadas como presupuesto base suplementario. En caso de rechazo, repliegue a Negro (Cancelada), reasignación y albarán provisional.
12. Principio operativo de mando unívoco: una cuadrilla, un supervisor asignado, eliminando conflictos de concurrencia.
13. Resolución de averías de flota en el Drawer: llamada a grúa en 1 clic, aviso automático a clientes de tareas pendientes por Telegram, desacoplamiento de órdenes para Drop & Go y gestión de transporte alternativo ante incompatibilidad de plazas.
14. Resolución de rotura de herramientas y piezas: traspaso condicional evaluado algorítmicamente según la finalización de tareas matinales en la hoja de faena de la Cuadrilla B, capa de proveedores locales con verificación telefónica de stock, e imputación de avería de herramienta prestada a quien ostenta la custodia física según Spec 004.
...
16. Protocolo Offline-First delimitado: rige para datos técnicos de la hoja de tarea y traspasos en IndexedDB cifrado con Web Crypto API; en incidencias urgentes el capataz busca activamente cobertura para reportar a la base; ante apagón total prolongado la cuadrilla prosigue en faena 100% offline.
...
18. Imputación objetiva de desviaciones no reportadas: asignadas al responsable de cuadrilla (capataz) para valoración por RRHH.
...
20. Trazabilidad multinivel del historial de incidencias: registrada en la hoja de tarea activa, replicada en el historial del vehículo en Flota (Spec 006) e imputada individualmente en RRHH (mecanismo formal de rectificación administrativa de conductor para siniestros viales, picking en almacén, capataz en faltas colectivas).
21. Funcionalidad "Drop and Go" como recurso de contingencias con Hoja de Consumo de Emergencia.
22. Circuito formal de imputación en PWA (foto inicio, piezas, horas, imputación de cancela, fotos de cierre).
23. Telemetría GPS eficiente (*Battery-Aware*) emitida exclusivamente por el capataz con apagado al fin de jornada y buffer de 25 minutos para alertas de pérdida de señal GPS en carretera.
24. Protocolo de continuidad de red técnica ante obstáculos no autorizados: by-pass provisional de emergencia facturando mano de obra y materiales consumidos a la orden, con soporte de RAG local.
25. Reactivación y transición de incidencias formalizada: el supervisor reactiva y resuelve en el Drawer web; el capataz cierra la tarea técnica en la PWA.
26. Tolerancia Cero a Datos Ficticios (*Zero Mock Data* con Estado Día 0 real centrado en base de empresa provista en onboarding).
27. Almacenamiento local soberano en disco duro del servidor (`/data/<empresa_id>/...` y `/docs/<empresa_id>/...`) garantizando privacidad y cumplimiento RGPD.
28. Rendimiento <200 ms y 60 fps estables garantizado mediante arquitectura asíncrona Celery + Redis para Whisper, Copilot y Telegram.
