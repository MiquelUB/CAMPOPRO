...
export default function CrearFeinaPage() {
  const [selectedJobType, setSelectedJobType] = useState<string | null>('jardineria');
          ...
          <div className="col-span-12 lg:col-span-7 flex flex-col gap-lg">
              ...
              <div className="grid grid-cols-2 gap-lg">
                <div className="col-span-2 flex flex-col gap-xs">
                  ...
                  <div className="relative group">
                    ...
                    <ChevronDown className="absolute right-md top-1/2 -translate-y-1/2 pointer-events-none text-outline w-5 h-5" />
                  </div>
                </div>
            ...
            <div className="flex flex-col gap-md">
              ...
              <div className="grid grid-cols-3 gap-md">
                <button 
                  onClick={() => setSelectedJobType('jardineria')}
                  className={`flex flex-col items-center gap-md p-lg bg-surface-container-lowest rounded-xl border-2 transition-all group overflow-hidden relative ${selectedJobType === 'jardineria' ? 'border-secondary-container shadow-md' : 'border-surface-container-highest hover:border-outline'}`}
                >
                  {selectedJobType === 'jardineria' && <div className="absolute inset-0 bg-secondary-container/5 opacity-100"></div>}
                  <TreePine className={`${selectedJobType === 'jardineria' ? 'text-secondary' : 'text-outline group-hover:text-primary'} w-10 h-10 relative z-10 transition-colors`} strokeWidth={1.5} />
                  <span className={`font-body-strong relative z-10 transition-colors ${selectedJobType === 'jardineria' ? 'text-on-surface' : 'text-on-surface-variant group-hover:text-on-surface'}`}>Jardineria</span>
                  {selectedJobType === 'jardineria' && <div className="absolute -bottom-2 -right-2 w-12 h-12 bg-secondary-container/10 rounded-full blur-xl group-hover:scale-150 transition-transform"></div>}
                </button>
                
                <button 
                  onClick={() => setSelectedJobType('muntatge')}
                  className={`flex flex-col items-center gap-md p-lg bg-surface-container-lowest rounded-xl border-2 transition-all group overflow-hidden relative ${selectedJobType === 'muntatge' ? 'border-secondary-container shadow-md' : 'border-surface-container-highest hover:border-outline'}`}
                >
                  {selectedJobType === 'muntatge' && <div className="absolute inset-0 bg-secondary-container/5 opacity-100"></div>}
                  <Hammer className={`${selectedJobType === 'muntatge' ? 'text-secondary' : 'text-outline group-hover:text-primary'} w-10 h-10 relative z-10 transition-colors`} strokeWidth={1.5} />
                  <span className={`font-body-strong relative z-10 transition-colors ${selectedJobType === 'muntatge' ? 'text-on-surface' : 'text-on-surface-variant group-hover:text-on-surface'}`}>Muntatge</span>
                  {selectedJobType === 'muntatge' && <div className="absolute -bottom-2 -right-2 w-12 h-12 bg-secondary-container/10 rounded-full blur-xl group-hover:scale-150 transition-transform"></div>}
                </button>
                
                <button 
                  onClick={() => setSelectedJobType('manteniment')}
                  className={`flex flex-col items-center gap-md p-lg bg-surface-container-lowest rounded-xl border-2 transition-all group overflow-hidden relative ${selectedJobType === 'manteniment' ? 'border-secondary-container shadow-md' : 'border-surface-container-highest hover:border-outline'}`}
                >
                  {selectedJobType === 'manteniment' && <div className="absolute inset-0 bg-secondary-container/5 opacity-100"></div>}
                  <Wrench className={`${selectedJobType === 'manteniment' ? 'text-secondary' : 'text-outline group-hover:text-primary'} w-10 h-10 relative z-10 transition-colors`} strokeWidth={1.5} />
                  <span className={`font-body-strong relative z-10 transition-colors ${selectedJobType === 'manteniment' ? 'text-on-surface' : 'text-on-surface-variant group-hover:text-on-surface'}`}>Manteniment</span>
                  {selectedJobType === 'manteniment' && <div className="absolute -bottom-2 -right-2 w-12 h-12 bg-secondary-container/10 rounded-full blur-xl group-hover:scale-150 transition-transform"></div>}
                </button>
              </div>
            </div>
          </div>
          ...
          <div className="col-span-12 lg:col-span-5 flex flex-col gap-lg">
            <div className="relative h-[420px] rounded-xl overflow-hidden shadow-xl group border-4 border-surface-container-lowest">
              ...
              <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-full flex flex-col items-center group-hover:-translate-y-[110%] transition-transform duration-300">
                ...
                <MapPin className="text-secondary-container w-12 h-12 drop-shadow-2xl fill-secondary-container" />
              </div>
            </div>
