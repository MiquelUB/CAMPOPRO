...
<input value={name} onChange={(e) => setName(e.target.value)} className="w-full h-[56px] px-4 rounded-xl bg-surface-container-highest text-on-surface font-body-md focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all" placeholder="Ex: Joan Garcia" type="text"/>
</div>
<div className="flex flex-col gap-2">
<label className="font-label-bold text-label-bold text-on-surface-variant ml-1">DNI / NIE</label>
<input value={dni} onChange={(e) => setDni(e.target.value)} className="w-full h-[56px] px-4 rounded-xl bg-surface-container-highest text-on-surface font-body-md focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all" placeholder="12345678X" type="text"/>
</div>
</div>
{/* Action Row */}
<div className="px-margin-mobile mt-stack-lg pb-stack-lg grid grid-cols-3 gap-3">
<button onClick={clearCanvas} className="col-span-1 h-[56px] rounded-xl bg-surface-container-high text-on-surface-variant font-button-text text-button-text flex items-center justify-center gap-2 active:scale-95 transition-transform" id="clear-btn">
<span className="material-symbols-outlined">delete</span>
            Esborrar
        </button>
<button onClick={handleConfirm} disabled={!hasSignature || !name || !dni || isSubmitting} className={`col-span-2 h-[56px] rounded-xl font-button-text text-button-text shadow-md flex items-center justify-center gap-2 transition-transform uppercase tracking-wide ${(!hasSignature || !name || !dni || isSubmitting) ? 'bg-surface-container text-on-surface-variant' : 'bg-secondary-container text-on-secondary-container active:scale-95'}`}>
<span className={`material-symbols-outlined ${isSubmitting ? 'animate-spin' : ''}`} style={{ fontVariationSettings: "'FILL' 1" }}>{isSubmitting ? 'sync' : 'check_circle'}</span>
            {isSubmitting ? 'CONFIRMANT...' : 'Confirmar'}
        </button>
</div>
</div>
</main>
